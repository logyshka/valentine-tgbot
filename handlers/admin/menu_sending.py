...
@dp.callback_query_handler(CallEqual("menu_sending"), IsAdmin())
...
@dp.callback_query_handler(CallEqual("start_sending"), IsAdmin())
...
@dp.message_handler(state=st.Sending.start)
...
@dp.callback_query_handler(CallEqual("accept_start_sending"), IsAdmin())
async def accept_start_sending(call: CallbackQuery):
    ...
    logger.success(f"id{call.from_user.id}: sending has been finished  {sending.success}-[OK]  {sending.failure}-[ERROR]")
    await call.message.delete()

@dp.callback_query_handler(CallEqual("add_button"), IsAdmin())
...
@dp.message_handler(lambda message: message.reply_to_message, state=st.AddButton.text)
async def start_sending(message: Message, state: FSMContext):
...
@dp.message_handler(state=st.AddButton.url)
async def start_sending(message: Message, state: FSMContext):
...
@dp.callback_query_handler(CallStart("accept_add_button"), state=st.AddButton.url)
