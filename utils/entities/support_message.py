...
from data import db_support_message
...
class SupportMessage:
    ...
    @property
    def from_user(self):
        data = db_support_message.get("from_id, from_username", unique_id=self.id)[0]
        return From(data[0], data[1])

    @property
    def sending_date(self):
        return datetime.datetime.fromisoformat(
            db_support_message.get("sending_date", unique_id=self.id)[0][0]).replace(microsecond=0)

    @property
    def text(self):
        return db_support_message.get("text", unique_id=self.id)[0][0]
    ...
    def delete(cls, unique_id):
        db_support_message.delete(unique_id=unique_id)
